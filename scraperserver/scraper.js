 ...
 async function scrap(search) {
  ...
  let data = [];
  let data_serco = await scrapSercoplus(search).catch(err => {throw err})
  let data_cyc = await scrapCyc(search).catch(err => {throw err})
  let data_impacto = await scrapImpacto(search).catch(err => {throw err})
  let data_memory = await scrapMemoryKings(search).catch(err => {throw err})
  let data_linio = await   scrapLinio(search).catch(err => {throw err})
  let data_vipasa = await   scrapVipasa(search).catch(err => {throw err})
  data = [... data_serco, ...data_cyc,...data_impacto,...data_linio,... data_memory,... data_serco, ...data_vipasa]
  console.log(data)
  return data;
}
...
async function scrapImpacto(buscar) {
  ...
  let data = await page.evaluate(() => {
    ...
    let url = document.querySelectorAll(".cajaprod_a");
